            ...
            for (let place of response.place) {
                ...
                if (place.linkGrp.ptr != null && typeof place.linkGrp.ptr[Symbol.iterator] === 'function') {
                    for (let ptr of place.linkGrp.ptr) {
                        folios += '<a href="briefe/'+ ptr['target'].replace('.xml', '.html') +'" style="display:block">'+ ptr['n'] +'</a>'
                    }
                } else {
                    folios += '<a href="briefe/'+ place.linkGrp.ptr['target'].replace('.xml', '.html') +'" style="display: block">'+ place.linkGrp.ptr['n'] +'</a>'
                }
